...
Making a role change observable exposed a pre-existing gap: `sites.conf` is re-read every loop, but the reload change-detection only compared the *set of URLs*, so a live edit that changed a site's *options* — its role, but also `timeout`/`tries` — was applied silently, with no confirmation it had loaded. The detection now compares each site's **full spec** (URL plus role/timeout/tries). Any live change is logged as a `Sites changed` line with the specific before→after (`… (role critical→advisory)`, `… (timeout 10s→25s)`), and a site newly switched to advisory has any active flaky-site advisory retired since it no longer gates. So an operator editing a site's config always gets confirmation the change took effect, whether or not the URL itself changed.

The same asymmetry existed for *bad* entries: the file was parsed loudly at startup (the preflight warns about a dropped unsafe URL or a warned-and-defaulted per-URL option) but the per-loop reload used a parse path that discarded those warnings — so a malformed edit made live was applied silently, the most dangerous case being a role typo that quietly falls back to `critical`. Both paths now go through one shared parse-and-warn helper, so a bad entry is surfaced identically whether it is present at startup or introduced by a live edit (deduped on reload — announced once when it appears, not every loop it persists).
